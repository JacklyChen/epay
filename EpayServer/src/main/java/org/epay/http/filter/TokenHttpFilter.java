...
public class TokenHttpFilter implements IHttpFilter {
	...
	public boolean httpFilter(HttpPacket httpPacket) throws HttpException {
		if (httpPacket.hSession.headParam.token == null) {
			if (HOpCode.LOGIN.equals(httpPacket.hSession.headParam.hOpCode) || HOpCode.VERIFY_NOTIFY.equals(httpPacket.hSession.headParam.hOpCode) || HOpCode.CREATE_ORDER_RECORD.equals(httpPacket.hSession.headParam.hOpCode)) {
				// 可以通过
				return true;
			} else {
